...
true <- c(mean0, psi0[row(psi0) >= col(psi0)] * 0.87, c(beta), mean.x1, mean.x2,
...
model_mx <- mxModel("Estimate a fixed knot with TICs", type = "RAM",
                    ...
                    mxData(observed = dat[, c(2:(2 * p + 3))], type = "raw"),
                    ...
                    mxPath(from = manifests, to = manifests, arrows = 2, free = T, values = init[17],
                           labels = "residuals"),
                    ...
                    mxPath(from = "x1", to = latents, arrows = 1, free = T, 
                           values = init[11:13], labels = paste0("beta1", 0:2)),
                    mxPath(from = "x2", to = latents, arrows = 1, free = T, 
                           values = init[14:16], labels = paste0("beta2", 0:2)),
                    ...
                    mxAlgebra(rbind(cbind(beta10, beta20),
                                    cbind(beta11, beta21),
                                    cbind(beta12, beta22)), name = "beta_s"),
...
model.se <- c(mean.se, model.para[model.para$name == "muknot", 3], 
              ...
              model.para[7, 3])
out0 <- data.frame(Name = paraFixedTIC, Estimate = model.est, SE = model.se)
out0$true <- true
out <- out0[c(1:5, 8, 10, 11:16), ]