...
GMM_mx1 <- mxModel("Growth Mixture Model, fixed knots", 
                   mxData(observed = dat, type = "raw"), class.list, classP, algebraObjective, objective)
Step1_model <- mxTryHard(GMM_mx1, extraTries = 9, loc = 1, scale = 0.05, 
                         initialGradientIterations = 20, OKstatuscodes = 0)

### Step 2:
############
#### Obtain estimates of step 1 ("correct" # of latent classes)
################################################################
step1.est <- list()
estimates <- summary(Step1_model)$parameters[, c(1, 5, 6)]
for (k in 1:c){
  step1.est[[k]] <- c(estimates[grep(paste0("c", k, "mueta"), estimates$name), 2],
                      estimates[grep(paste0("c", k, "muknot"), estimates$name), 2],
                      estimates[grep(paste0("c", k, "psi0s"), estimates$name), 2],
                      estimates[grep(paste0("c", k, "psi1s"), estimates$name), 2],
                      estimates[grep(paste0("c", k, "psi2s"), estimates$name), 2],
                      estimates[grep(paste0("c", k, "psigg"), estimates$name), 2],
                      estimates[grep(paste0("c", k, "residuals"), estimates$name), 2])
}
...
for (k in 1:c){
  ...
  class.list[[k]] <- mxModel(name = paste0("Class", k), type = "RAM", 
                             ...
                             mxPath(from = "eta2s", to = manifests, arrows = 1, free = F, values = 0, 
                                    labels = paste0("c", k, "L2", 1:p, "[1,1]")),
                             #### Define the variances of residuals
                             mxPath(from = manifests, to = manifests, arrows = 2, free = F, values = step1.est[[k]][11],
                                    labels = paste0("c", k, "residuals")),
                             #### Define means of latent variables
                             mxPath(from = "one", to = latents[1:3], arrows = 1, free = F, values = step1.est[[k]][1:3],
                                    labels = paste0("c", k, c("mueta0s", "mueta1s", "mueta2s"))),
                             #### Define var-cov matrix of latent variables
                             mxPath(from = latents, to = latents, arrows = 2,
                                    connect = "unique.pairs", free = F,
                                    values = step1.est[[k]][c(5:10)],
                                    labels = paste0("c", k, c("psi0s0s", "psi0s1s", "psi0s2s", 
                                                              "psi1s1s", "psi1s2s", "psi2s2s"))),
                             #### Add additional parameter and constraints
                             mxMatrix("Full", 1, 1, free = F, values = step1.est[[k]][4], 
                                      labels = paste0("c", k, "muknot"), name = paste0("c", k, "mug")),
                             outDef, outLoads1, outLoads2, 
                             mxAlgebraFromString(paste0("rbind(c", k, "mueta0s, c", k, "mueta1s, c", k, "mueta2s)"), 
                                                 name = paste0("c", k, "mean_s")),
...
objective <- mxFitFunctionML()
GMM_mx2 <- mxModel("Growth Mixture Model, random knots", 
                   mxData(observed = dat, type = "raw"), 
                   class.list, classBeta, classPV, classP, algebraObjective, objective)
Step2_model <- mxTryHard(GMM_mx2, extraTries = 9, loc = 1, scale = 0.05, 
                         initialGradientIterations = 20, OKstatuscodes = 0)
...
model.para <- summary(Step1_model)$parameters[, c(1, 5, 6)]
### Estimates of parameters in each latent class
model.est <- model.se <- est <- list()
for (k in 1:c){
  model.est[[k]] <- c(mxEvalByName(paste0("c", k, "mean"), model = Step1_model@submodels[[k]]),
                      model.para[model.para$name == paste0("c", k, "muknot"), 2],
                      mxEvalByName(paste0("c", k, "psi"), model = Step1_model@submodels[[k]])[
                        row(mxEvalByName(paste0("c", k, "psi"), model = Step1_model@submodels[[k]])) >= 
                          col(mxEvalByName(paste0("c", k, "psi"), model = Step1_model@submodels[[k]]))],
                      model.para[model.para$name == paste0("c", k, "residuals"), 2])
  model.se[[k]] <- c(mxSE(paste0("Class", k, ".c", k, "mean"), Step1_model, forceName = T), 
                     model.para[model.para$name == paste0("c", k, "muknot"), 3],
                     mxSE(paste0("Class", k, ".c", k, "psi"), Step1_model, forceName = T)[
                       row(mxSE(paste0("Class", k, ".c", k, "psi"), Step1_model, forceName = T)) >=
                         col(mxSE(paste0("Class", k, ".c", k, "psi"), Step1_model, forceName = T))],
...
Step1.out0 <- rbind(do.call(rbind.data.frame, est), est.weights)
Step1.out0$True <- c(c1_true, c2_true, exp(Beta[2, 1]))
Step1.out <- Step1.out0[c(1:5, 8, 10, 12:16, 19, 21), ]

Step2.out <- data.frame(Name = paste0("beta", rep(2:c, 3), rep(0:2, each = c - 1)), 
                        Estimate = c(mxEval(classbeta, Step2_model)[-1, ]),
                        SE = c(mxSE(classbeta, Step2_model)[-1, ]))
Step2.out$True <- c(Beta[-1, ])
